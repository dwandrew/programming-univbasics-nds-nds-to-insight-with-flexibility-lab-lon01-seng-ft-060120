...
other_movies= []
  while index<movies_collection.length
    #updated_movies.push({:director_name => name})
    brian = {:director_name => name, :title => movies_collection[index]}
...
else gross_array = {collection[index][:studio] => collection[index][:worldwide_gross]}
end
index+=1
end
p gross_array
...
while i<source.length
  ...
  while inner<source[i][:movies].length
    directors_movies.push([:title => source[i][:movies][inner][:title], :director_name => source[i][:name], :worldwide_gross => source[i][:worldwide_gross]])
    inner+=1
  end
  i+=1;
end
